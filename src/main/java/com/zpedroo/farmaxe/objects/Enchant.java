...
public class Enchant {
    ...
    public List<String> getReplacers(@NotNull ItemStack item) {
        ...
        int level = FarmAxeUtils.getEnchantmentLevel(item, this);
        ...
        for (EnchantProperty property : EnchantProperty.values()) {
            replacers.add(NumberFormatter.getInstance().formatDecimal(FarmAxeUtils.getEnchantEffectByLevel(this, property, level, quality)));
            replacers.add(NumberFormatter.getInstance().formatDecimal(FarmAxeUtils.getEnchantEffectByLevel(this, property, nextLevel, quality)));
        }
