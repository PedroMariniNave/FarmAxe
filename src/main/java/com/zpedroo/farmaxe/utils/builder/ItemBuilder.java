package com.zpedroo.farmaxe.utils.builder;
        ...
        if (file.contains(where + ".enchants")) {
            for (String str : file.getStringList(where + ".enchants")) {
                ...
                if (StringUtils.contains(enchantment, ",")) {
                    ...
                    int level = Integer.parseInt(StringUtils.replaceEach(split[1], placeholders, replacers));
                    builder.addEnchantment(Enchantment.getByName(split[0]), level);
                } else {
